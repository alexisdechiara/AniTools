...
<template>
	<UDashboardPanel id="home">
		<div class="flex flex-col px-4 sm:px-6 pt-4 sm:pt-6">
			<span class="font-semibold text-highlighted text-4xl">Hey alexisdechiara 👋</span>
			<span class="text-dimmed font-light text-2xl">Here all your summary</span>
		</div>
		<div class="relative">
			<GridLayout :layout="layout" :margin="[16, 16]" :row-height="104" class="size-full absolute inset-0">
				<GridItem v-for="item in layout" :key="item.i" :x="item.x" :y="item.y" :w="item.w" :h="item.h" :i="item.i"
					drag-allow-from=".draggable-grip">
					<template v-if="getComponentById(item.componentId)">
						<component :is="getComponentById(item.componentId)" v-bind="item.componentProps" draggable
							class="size-full" />
					</template>
					<UAlert v-else icon="i-lucide-alert-octagon" title="Error"
						description="An error occurred while loading the card" variant="subtle" color="error"
						class="draggable-grip size-full" />
				</GridItem>
			</GridLayout>
		</div>
		<UButton icon="i-lucide-plus" label="Add a card"
			class="m-4 ring-0 border border-dashed cursor-pointer flex items-center justify-center py-4" variant="outline"
			size="xl" />
	</UDashboardPanel>
</template>
